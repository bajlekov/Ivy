        ...
        *self.dependencies.borrow_mut() = vec![]; // clear the dependency stack
        ...
        if self.generated_constants.borrow().is_none() {
            ...
            for (name, expr) in self.constants.borrow().iter() {
                consts.push_str(&format!("const {}", self.gen_var(name, expr)?));
            }
        ...
        if let Some(Stmt::Kernel { id, args, body }) = self.kernels.borrow().get(name) {
            ...
            for (idx, arg) in args.iter().enumerate() {
                ...
                let arg_out = format!(
                    "{} {}{}",
                    match input[idx] {
                        VarType::Int | VarType::IntArray(1, ..) => "uniform int",
                        VarType::Buffer { .. } | VarType::Float | VarType::FloatArray(1, ..) =>
                            "uniform float",
                        err_type =>
                            return Err(format!(
                    ...
                    arg,
                    match input[idx] {
                        VarType::Buffer { .. } => format!("[], uniform int ___str_{}[]", arg),
                        VarType::Int | VarType::Float => "".into(),
                        VarType::IntArray(1, ..) | VarType::FloatArray(1, ..) => "[]".into(),
                        err_type =>
                            return Err(format!(
                                "Type '{}' of argument '{}' not supported in kernel arguments",
                                err_type, arg
...
",
            ...
            for (idx, arg) in args.iter().enumerate() {
                ...
                match input[idx] {
                    VarType::Buffer { .. } => kernel.push_str(&format!("{}, ___str_{}", arg, arg)),
                    _ => kernel.push_str(arg),
                }
            }
            kernel.push_str(");\n}");
                            ...
                            if let VarType::Buffer { z, cs, x1y1 } = var {
                                ...
                                if z == 3 {
                                    let id_x = format!(
                                        "{}[(varying int)({})]",
                                        name,
                                        var.idx_3d(name, &i1, &i2, "0")
                                    );
                                    let id_y = format!(
                                        "{}[(varying int)({})]",
                                        name,
                                        var.idx_3d(name, &i1, &i2, "1")
                                    );
                                    let id_z = format!(
                                        "{}[(varying int)({})]",
                                        name,
                                        var.idx_3d(name, &i1, &i2, "2")
                                    );
                ...
                if let Expr::Identifier(name) = &**expr {
                    match var {
                        ...
                        VarType::Buffer { x1y1, .. } => {
                            ...
                            let id = format!("{}[(varying int)({})]", name, var.idx_1d(name, &i1));
                            format!("{} {} = {};\n", guard, id, val)
                        }
                        err_type => {
                            return Err(format!(
