...
kernel void area_clamp(
  ...
  float p9 = $I[x + 1, y + 1, z];
	...
	float m1 = min(min(min(min(min(min(min(p1, p2), p3), p4), p6), p7), p8), p9);
	float m2 = max(max(max(max(max(max(max(p1, p2), p3), p4), p6), p7), p8), p9);

	$O[x, y, z] = clamp(p5, m1, m2);
}
