...
kernel void dist(global float *in, global float *t1, const int ox, const int oy) {
  ...
  float i1 = $in[x, y, 0];
  float i2 = $in[x+ox, y+oy, 0];

  // photon well depth for proper scaling of the poisson noise
  float depth = 25000.0f;

  // anscombe transform from poisson noise to gaussian noise
  i1 = 2.0f*sqrt(i1*depth + 3.0f/8.0f);
  i2 = 2.0f*sqrt(i2*depth + 3.0f/8.0f);

  float o = pown(i1 - i2, 2)*3.0f;
  $t1[x, y, 0] = o;
}
...
kernel void accumulate(global float *in, global float *t1, global float *t3, global float *t4, global float *wmax, global float *p1, const int ox, const int oy) {
  const int x = get_global_id(0);
  const int y = get_global_id(1);

  float sigma = pown(1.0f - $p1[x, y, 0], 5);
  float pf = exp(-$t1[x, y, 0]*sigma);
  float nf = exp(-$t1[x-ox, y-oy, 0]*sigma);

  float pi = $in[x+ox, y+oy, 0];
  float ni = $in[x-ox, y-oy, 0];

  float o = $t3[x, y, 0] + pi*pf + ni*nf;
  float f = $t4[x, y, 0] + pf + nf;

  float w = $wmax[x, y, 0];
  $wmax[x, y, 0] = fmax(w, fmax(pf, nf));

  $t3[x, y, 0] = o;
  $t4[x, y, 0] = f;
}

kernel void norm(global float *in, global float *t3, global float *t4, global float *wmax, global float *out) {
  const int x = get_global_id(0);
  const int y = get_global_id(1);

  float i = $in[x, y, 0];
  float w = $wmax[x, y, 0];
  float o = (w*i + $t3[x, y, 0]) / (w + $t4[x, y, 0]);
